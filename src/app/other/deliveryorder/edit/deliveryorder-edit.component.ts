...
export class DeliveryOrderEditComponent implements OnInit {
    ...
    Allvalid() {
        ...
        // }
        /*
                if (PICK_CATEGORY == "OTHERS" || PICK_CATEGORY == "EXTRA")
                {
                    if (Txt_Truker_Code.IsBlank())
                    {
                        MessageBox.Show("Trucker Code cannot be empty", "Save", MessageBoxButton.OK);
                        Txt_Truker_Code.TxtLovBox.Focus();
                        return false;
                    }
                    if (Txt_Truker.IsBlank())
                    {
                        MessageBox.Show("Trucker Name cannot be empty", "Save", MessageBoxButton.OK);
                        Txt_Truker.Focus();
                        return false;
                    }
                    if (Txt_Pickup.IsBlank())
                    {
                        MessageBox.Show("Pick-up  cannot be empty", "Save", MessageBoxButton.OK);
                        Txt_Pickup.Focus();
                        return false;
                    }
                    if (Txt_Pick_Addr1.IsBlank())
                    {
                        MessageBox.Show("Pick-up address cannot be empty", "Save", MessageBoxButton.OK);
                        Txt_Pick_Addr1.Focus();
                        return false;
                    }
                    if (Txt_To_Name.IsBlank())
                    {
                        MessageBox.Show("To Name cannot be empty", "Save", MessageBoxButton.OK);
                        Txt_To_Name.Focus();
                        return false;
                    }
                    if (Txt_To_Addr1.IsBlank())
                    {
                        MessageBox.Show("To address cannot be empty", "Save", MessageBoxButton.OK);
                        Txt_To_Addr1.Focus();
                        return false;
                    }
                    if (cmb_Danger_Goods.SelectedItem == null)
                    {
                        MessageBox.Show("Dangerous goods not selected", "Save", MessageBoxButton.OK);
                        cmb_Danger_Goods.Focus();
                        return false;
                    }
                    if (cmb_Term_shipment.SelectedItem == null)
                    {
                        MessageBox.Show("Terms of shipment not selected", "Save", MessageBoxButton.OK);
